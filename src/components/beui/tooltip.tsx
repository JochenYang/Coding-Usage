              ...
              {open && coords ? (
                ...
                >
                  <motion.span
                    ...
                    className={cn(
                      // bg-card (not the upstream bg-background): the page
                      // background IS --color-background, so a panel painted
                      // with it vanished into the page in the light theme.
                      // Every other floating surface (popover/drawer/our own
                      // Tooltip) uses card/popover for exactly this reason.
                      "block whitespace-nowrap rounded-lg border border-border bg-card px-2.5 py-1 text-xs font-medium text-foreground shadow-lg",
                      className,
                    )}
                  >
                    {content}
